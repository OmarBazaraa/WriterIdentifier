...
from src.utils.iam_dataset import IAMDataset, resize_image
from src.utils.iou_loss import IOU_loss
from src.utils.draw_box_on_image import draw_box_on_image
...
def make_cnn(ctx=mx.gpu()):
    ...
    with body.name_scope():
        first_layer = gluon.nn.Conv2D(channels=64, kernel_size=(7, 7), padding=(3, 3), strides=(2, 2), in_channels=1,
                                      use_bias=False)
...
if __name__ == "__main__":
    ...
    if restore_checkpoint_name:
        ...
        assert os.path.isfile(restore_checkpoint), "{} does not exist".format(
            os.path.join(checkpoint_dir, restore_checkpoint_name))
    main()
